...
function QuizModal({ courseId, quizSetId, quizzes, open, setOpen }) {
  ...
  const submitQuiz = async (event) => {
    try {
      ...
      toast.error("Problem in submitting the quiz", error);
    }
  };
          ...
          <div className="grid md:grid-cols-2 gap-5 mb-6">
            {currentQuiz?.options.map((option) => (
              <div key={option.label}>
                ...
                />
                <Label
                  className="border border-border rounded px-2 py-3 block cursor-pointer hover:bg-gray-50 transition-all font-normal peer-checked:bg-green-100"
